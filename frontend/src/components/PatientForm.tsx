    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const patientPayload = {
                ...
                autopsy: formData.autopsy || null,
                record_status: formData.record_status || '0',
                check_status: formData.check_status || 'Unchecked',
                workflow: role === 'medecin' ? 'clinique' : role === 'anapate' ? 'anapath' : 'valide',
            };
            
            // 2. Submit patient to /api/patients/
            const patientRes = await axios.post('patients/', patientPayload);
            const createdPatient = patientRes.data;
            const patientId = createdPatient.patient_id;
            
            // 3. Prepare tumor fields payload (linked to the patient)
            const tumorPayload = {
                patient: patientId,
                incidence_date: formData.incidence_date || new Date().toLocaleDateString('fr-FR'),
                topo_code: formData.topo_code || 'C80.9',
                morpho_code: formData.morpho_code || '8000/3',
                behaviour: formData.behaviour || '3',
                grade: formData.grade || '9',
                basis_of_diagnosis: formData.basis_of_diagnosis || '9',
                laterality: formData.laterality || '0',
                clinical_t: formData.clinical_t || null,
                clinical_n: formData.clinical_n || null,
                clinical_m: formData.clinical_m || null,
                clinical_stage_group: formData.clinical_stage || null,
                pathological_t: formData.path_t || null,
                pathological_n: formData.path_n || null,
                pathological_m: formData.path_m || null,
                notes: formData.notes || '',
            };
            
            // 4. Submit tumor to /api/tumors/
            const tumorRes = await axios.post('tumors/', tumorPayload);
            
            // 5. If we have sources data, submit it too!
            if (formData.source_type || formData.hospital_name || formData.department || formData.report_number) {
                const sourcePayload = {
                    tumor: tumorRes.data.id,
                    source_type: formData.source_type || 'CL',
                    source_description: `Rapport importé pour ${formData.last_name}`,
                    hospital_name: formData.hospital_name || null,
                    department: formData.department || null,
                    report_number: formData.report_number || null,
                    practitioner_name: formData.practitioner_name || null,
                    reader_id: formData.reader_id || null,
                    clinical_text: formData.clinical_text || null,
                    pathology_text: formData.pathology_text || null,
                };
                await axios.post('sources/', sourcePayload);
            }
            
            setSuccess(true);
        } catch (err: any) {
            console.error("Error creating patient document", err);
            alert(err.response?.data ? JSON.stringify(err.response.data) : "Erreur de connexion au serveur.");
        } finally {
            setLoading(false);
        }
    };
